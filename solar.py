...
class space:
    ...
    def __init__(self, width = 2400, height = 1200):
        ...
        self.sun_L = Label(self.window, text = "sun", font = "TkDefaultFont 20 bold")
...
class force:
    ...
    def sum_force(self, total, dir):
        self.total = total
        self.dir = dir
        self.x = total * math.cos(dir)
        self.y = total * math.sin(dir)
        
    def sep_force(self, x, y):
        self.x = x
        self.y = y
...
class gravity(force):
    def __init__(self, planet, target):
        ...
        try:
            direction = math.atan((planet.y-target.y)/(planet.x-target.x))#planet
...
class planet:
    ...
    def move(self):
        ...
        self.canvas.after(25,self.canvas.delete, self.canvas.create_text(self.x_pos, 
                                                                          self.y_pos, text = self.name, anchor = 'sw'))
        self.canvas.move(self.id, movement_x * r_scale, -movement_y * r_scale)
